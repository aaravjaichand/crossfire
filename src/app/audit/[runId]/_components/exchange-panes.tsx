...
// stream turns in; once the referee rules on it the sample stops polling.
export function ExchangePanes({ runId, sample }: { runId: string; sample: SampleView }) {
  ...
  useEffect(() => {
    ...
    async function tick() {
      try {
        ...
        if (!res.ok) return;
        const next: SampleView = await res.json();
  ...
  }, [runId, sample.id, polling]);

  const latestEvidence = [...live.thread].reverse().find((m) => m.evidence)?.evidence;
...
function Message({ message }: { message: MessageView }) {
  ...
  return (
    <article className={`border-l-2 pl-3 ${role.accent}`}>
      ...
      <p className="mt-1 text-[13px] leading-relaxed text-neutral-300">{message.content}</p>
